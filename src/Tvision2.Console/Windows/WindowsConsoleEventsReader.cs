...
public class WindowsConsoleEventsReader : IConsoleEventsReader
{
    ...
    public WindowsConsoleEventsReader()
    {
    ...
    public void ReadEvents(TvConsoleEvents events)
    {
        ...
        if (numEvents > 0)
        {
            ...
            WindowsNative.ReadConsoleInput(_hstdin, ref MemoryMarshal.GetReference(byteBuf), numEvents,
                out var eventsRead);
            AddWin32Events(events, buffer);
        }
    }
    ...
    private TvConsoleMouseEvent CreateMouseEventFromWin32(in MOUSE_EVENT_RECORD recordMouseEvent)
    {
        ...
        var evt = new TvConsoleMouseEvent(recordMouseEvent.dwMousePosition.X, recordMouseEvent.dwMousePosition.Y,
            states);
        return evt;
    }
